...
public class JoinQuery implements Serializable{
	 ...
	 * @param pointRDD 
	 * @param rectangleRDD
	 * @return JavaPairRDD in the required format
	 */
	public JavaPairRDD<Envelope, HashSet<Point>> SpatialJoinQueryUsingCartesianProduct(PointRDD pointRDD, RectangleRDD rectangleRDD) {
		...
		List<Envelope> envelopes = rectangleRDD.rawRectangleRDD.collect();
		for(Envelope env : envelopes) {
			List<Point> listPoints = RangeQuery.SpatialRangeQuery(pointRDD, env, 0).rawPointRDD.collect();
			HashSet<Point> hsPoints = new HashSet<>(listPoints);
			Tuple2<Envelope, HashSet<Point>> tuples = new Tuple2<Envelope, HashSet<Point>>(env, hsPoints);
			resultList.add(tuples);
		}
		return aggregateJoinResultPointByRectangle(sc.parallelizePairs(resultList));
	}
